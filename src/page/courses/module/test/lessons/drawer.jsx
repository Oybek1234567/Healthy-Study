...
const LessonsDrawer = ({ open, onClosed, onCreate }) => {
    ...
    const handlePost = async () => {
        ...
        try {
            ...
            const res = await axios.post("http://localhost:3000/lessons/create", data);
            onCreate(data.name);
    ...
    <div>
          <Drawer open={open} onClose={onClosed}>
              <Form onFinish={handlePost}>
                  ...
                  <button type="submit" className="bg-green-800 p-2 mt-3 text-white ml-3 rounded-lg">Create</button>
            </Form>
      </Drawer>
    </div>
  )
